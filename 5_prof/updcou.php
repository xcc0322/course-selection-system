...
if(gettype($_POST['time'])=='array'){
	$coutime = trans_back($_POST['time']);
		...
		while($list3BB=mysql_fetch_array($sql2BB)) {
			$stu = $list3BB['sid'];
			$sql = "select * from student where sid = '$stu';";
			$res = mysql_query($sql);
			while($list=mysql_fetch_array($res)) {
				if (!($cougrade & (1<<($list['grade']-1)))) $info = "$stu, You are not in the right grade for Course $cid now!";
				if ($list['did']!=$coudept) $info = "You are not in the right department for Course $cid now!";
				if ($info != "success") {note($stu,$cid,$info);$info = "success";}
			}
			$sql = "select * from take where sid = '$stu' and cid!='$cid';";
			$res = mysql_query($sql);
			while($list=mysql_fetch_array($res)) {
				$mycid = $list['cid'];
				$temp = mysql_fetch_array(mysql_query("select class_time from course where cid = '$mycid'"));
				if (conflict($coutime,$temp[0])) $info = "Course $cid conflicts with another course now!";
				if ($info != "success") {note($stu,$cid,$info);$info = "success";}
			}
		}
		$sqla = "update course set name = '$coursename', grade=$cougrade,did='$coudept',
